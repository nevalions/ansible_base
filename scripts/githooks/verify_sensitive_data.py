            ...
            for match in re.finditer(pattern, line):
                ...
                if pattern_type == "sensitive_key":
                    if (
                        "vault_" in line
                        and "pass:" in line
                        and ("r'" in line_content or 'r"' in line_content)
                    ):
                        continue
...
def print_violations(violations):
    ...
    for filepath, file_violations in sorted(by_file.items()):
        ...
        for v_type, v_list in sorted(by_type.items()):
            ...
            for v in sorted(v_list, key=lambda x: x["line"]):
                print(
                    f"    {COLORS['RED']}Line {v['line']}: {v['message']}{COLORS['NC']}"
                )
        print()
    ...
    print(
        f"{COLORS['YELLOW']}Use 'git commit --amend' to edit or 'git reset HEAD <file>' to unstage files.{COLORS['NC']}\n"
    )
